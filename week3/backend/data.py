"""
Precomputes demand aggregations from the upstream demand parquet at startup.
Week 3: treats the upstream data as potentially-corrupted (dedupe + clip on
load, and run a validation pass that logs every issue without crashing).
"""
...
_BACKEND_DIR = Path(__file__).resolve().parent          # .../week3/backend
_WEEK3_DIR = _BACKEND_DIR.parent                        # .../week3
_REPO_ROOT = _WEEK3_DIR.parent                          # repo root

# Week 3 paths. The upstream demand parquet is treated as untrusted input:
# load it, clean it, log what was wrong, keep serving.
DATA_PATH = _WEEK3_DIR / "data" / "demand_enriched_corrupted.parquet"
CORRUPTED_DATA_PATH = DATA_PATH  # explicit alias for the validation check

# Fall back to the week-2 location for lookups + model + small parquet so this
# file also works when run in a context that only has the week-2 layout.
def _first_existing(*candidates):
    for p in candidates:
        if Path(p).exists():
            return Path(p)
    return Path(candidates[0])  # return the first as a "best-effort" default

LOOKUP_PATH = _first_existing(
    _WEEK3_DIR / "metadata" / "Lookups" / "taxi_zone_lookup.csv",
    _REPO_ROOT / "week2" / "metadata" / "Lookups" / "taxi_zone_lookup.csv",
    Path("/metadata/Lookups/taxi_zone_lookup.csv"),
)
MODEL_PATH = _first_existing(
    _WEEK3_DIR / "model" / "lgbm_demand_model.txt",
    Path("/data/processed/lgbm_demand_model.txt"),
)

# Sentinel values used by the cleaning step.
TRIP_COUNT_MIN = 0
TRIP_COUNT_MAX = 500   # baseline historical max is 310; ceiling is generous
...
def _clean_upstream(df: pd.DataFrame) -> pd.DataFrame:
    """Graceful degradation: dedupe + clip out-of-range trip_count.

    Bad rows are repaired in-place; every action is logged so operators have
    visibility. The API never crashes on bad upstream data — it just keeps
    serving with cleaned values.
    """
    n_before = len(df)

    # 1. Dedupe on (PULocationID, time_bucket) key — keep first occurrence.
    if {"PULocationID", "time_bucket"}.issubset(df.columns):
        key_dupes = df.duplicated(subset=["PULocationID", "time_bucket"]).sum()
        if key_dupes:
            logger.warning(
                "Dropped %d duplicate (PULocationID, time_bucket) rows during load.",
                int(key_dupes),
            )
            df = df.drop_duplicates(subset=["PULocationID", "time_bucket"], keep="first")

    # 2. Clip trip_count to a sane range. Out-of-range = corruption.
    if "trip_count" in df.columns:
        neg = int((df["trip_count"] < TRIP_COUNT_MIN).sum())
        hi  = int((df["trip_count"] > TRIP_COUNT_MAX).sum())
        if neg or hi:
            logger.warning(
                "Clipping trip_count: %d below %d, %d above %d (out of %d rows).",
                neg, TRIP_COUNT_MIN, hi, TRIP_COUNT_MAX, len(df),
            )
            df = df.copy()
            df["trip_count"] = df["trip_count"].clip(TRIP_COUNT_MIN, TRIP_COUNT_MAX)

    if len(df) != n_before:
        logger.info("Upstream rows after cleaning: %d (was %d).", len(df), n_before)
    return df
...
def _load():
    ...
    try:
        ...
        # Graceful degradation: if the parquet is unreadable we still want the
        # API to start. Return an empty profile + zones_df with the right shape.
        logger.error("Could not read %s (%s) — starting with empty profile.", DATA_PATH, e)
        empty_profile = pd.DataFrame(
            columns=["PULocationID", "hour", "dayofweek", "avg", "holiday_name"]
        )
        try:
            ...
            logger.error("Could not read lookup %s (%s) — using empty zones table.", LOOKUP_PATH, ze)
            zones_df = pd.DataFrame(columns=["zone_id", "name", "borough", "service_zone"])
        return empty_profile, zones_df

    df = _clean_upstream(df)
...
def check_and_log_data_quality():
    """Run the week-3 validator against the upstream parquet and log issues.

    Never crashes — every failure path is logged. Called at module import so
    operators see data quality alerts immediately on pod startup.
    """
    try:
        # Import lazily so this module can be imported even when the validation
        # package isn't on sys.path (e.g. when the file is read from /app/).
        sys.path.insert(0, str(_REPO_ROOT))
        from week3.validation.check_data_quality import (
            DataQualityValidator, split_baseline_and_current, format_report,
        )
    except Exception as e:
        logger.error("Could not import data-quality validator (%s).", e)
        return
    ...
    try:
        ...
        logger.warning("Data quality parquet not found at %s — skipping check.", CORRUPTED_DATA_PATH)
        return
    except Exception as e:
    ...
    severity = "warning" if result["is_valid"] else "error"
    log_fn = logger.warning if severity == "warning" else logger.error
    log_fn("Data quality check: %d issue(s) (is_valid=%s).",
           result["num_issues"], result["is_valid"])
    for issue in result["issues"]:
        log_fn("  [%s] %s: %s", issue["severity"].upper(), issue["type"], issue["description"])


_profile, _zones_df = _load()
_zone_map = _zones_df.set_index("zone_id").to_dict("index") if not _zones_df.empty else {}
...
def _load_zone_hour_fares():
    ...
    try:
        fare_path = _first_existing(
            _BACKEND_DIR / "zone_hour_avg_fare.parquet",
            _REPO_ROOT / "data" / "processed" / "zone_hour_avg_fare.parquet",
            Path("/data/processed/zone_hour_avg_fare.parquet"),
        )
...
def _load_zone_coordinates():
    ...
    try:
        geojson_path = _first_existing(
            _BACKEND_DIR / "taxi_zones.geojson",
            _REPO_ROOT / "app" / "frontend" / "public" / "taxi_zones.geojson",
            Path("/app/frontend/public/taxi_zones.geojson"),
        )
